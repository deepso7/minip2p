...
4. **`std` adapters**: the `std`-gated mDNS socket adapter (`MdnsSockets`, one implementation of the portable `MdnsIo` seam), `crates/minip2p` — the application-facing `Endpoint` API, which composes the transports it is asked to bind into a `TransportSet` and routes by address, and whose features layer on without changing the base API — and `crates/ffi`, the UniFFI endpoint/driver adapter for foreign runtimes.
