...
1. **Sans-I/O protocol crates** (`no_std + alloc`), one per protocol: `multistream-select`, `ping`, `identify`, `relay`, `autonat`, `dcutr`, `pubsub`, `mdns`; plus `identity`, `core`, `tls`, `platform` (`Clock`/`Now`/`Deadline`/`EntropySource` contracts), `secure-mux` (multistream-select + Noise XX + Yamux over an ordered byte stream, shared by circuit and TCP), and `transport` (the `Transport`/`BlockingTransport` contracts, plus `TransportSet`, which routes several transports behind one of them).
