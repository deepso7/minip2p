...
The state machines typically run over a stream that was originally a relay circuit bridge. In minip2p that is exposed as a user protocol on the swarm (`swarm.add_protocol(DCUTR_PROTOCOL_ID)` + `Stream*` events). The end-to-end pure-state-machine flow is exercised by `crates/swarm/tests/relay_holepunch_flow.rs`.
