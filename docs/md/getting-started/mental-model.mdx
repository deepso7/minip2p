---
title: Mental model
description: Understand endpoints, peer identities, addresses, events, and the difference between direct dialing and NAT-aware connecting.
---

Five ideas explain most application code built on minip2p.
...
facade supplies sockets and clocks so application code can stay small, while
...
driver on `EndpointBuilder`; see [Installation](/getting-started/installation#compiled-is-not-enabled).
:::
...
Continue with [Listen and connect](/guides/listen-and-connect) or compare all
features in the [feature matrix](/reference/feature-matrix).
