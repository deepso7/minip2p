---
title: Custom protocols and streams
description: Register an application protocol, negotiate streams, exchange bytes, and close or abandon stream state intentionally.
---
...
an ID in `RESERVED_PROTOCOL_IDS` fails with `SwarmError::ReservedProtocol`.
:::
...
stream. It is idempotent and prevents an ignored stream from filling focused
wait buffers.
...
Continue with [Events and timeouts](/guides/events-and-timeouts).
