...
**Goal:** Turn on discovery, observe the known-peer book, and let the driver
dial peers from signed beacons.

**Assumes:** [Publish with pubsub](/guides/pubsub). Relayed paths still need a
configured relay — see [Traverse NAT](/guides/traverse-nat).

<Badge>pre-1.0</Badge> <Badge>implies nat + pubsub</Badge>
