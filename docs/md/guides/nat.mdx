...
let path = node
    .wait_path(connect_id, Duration::from_secs(60))?
    .ok_or("no usable path before the deadline")?;

println!("connected via {path:?}");
```

`wait_path` consumes the matching `NatEvent::PathEstablished`. If the attempt
fails, it returns `None` and leaves `NatEvent::ConnectFailed` queued so the
application can inspect the reason with `take_nat_events`.
