...
- Direct connections: `connect`, `cancel_connect`, `dial`, `dial_ip4`, `dial_ip6`, `disconnect`
...
- Driving: focused waits `wait_peer_ready`, `nat_wait_path` (nat), `wait_ping_rtt`; event loop `wait` (every event, including capability variants); migration `next_event`, `next_wake`; non-blocking `poll`
- Shutdown: `close`

Available on `EndpointBuilder` with `quic`:

- Bind: `listen_on`, `listen_on_multiaddr`, `listen_default`, `quic`, `quic_multiaddr`, `quic_dual_multiaddr`, `quic_dual_stack`, `bind_quic`, `bind_quic_multiaddr`, `bind_quic_dual_stack`
...
- Bind: `listen_on`, `listen_on_multiaddr`, `tcp`, `tcp_multiaddr`, `bind_tcp`
- Configuration: `tcp_config`
- Routing: QUIC addresses use QUIC; `/tcp` dials use TCP

## Portable API

With default features off, `Endpoint::portable(identity, entropy)` starts a `PortableEndpoint` builder. Call `build(transport)` with the caller-provided `Transport`. The resulting endpoint uses `poll(now)` and `next_deadline(now)` instead of the std endpoint's blocking wait methods; `poll(now)` returns `Vec<EndpointEvent>`, including enabled capability variants. It never reads a system clock or starts a runtime.
...
- Attempts: `connect`, `cancel_connect`, `nat_wait_path` (migration; removed in #181)
- Events: `EndpointEvent::Nat(..)` via `wait` (migration: `take_nat_events`, `next_nat_event`)
...
- Events: `EndpointEvent::Gossipsub(..)` via `wait` (migration: `take_gossipsub_events`, `next_gossipsub_event`)
...
- State: `known_peers`
- Events: `EndpointEvent::Discovery(..)` via `wait` (migration: `take_discovery_events`, `next_discovery_event`)
