...
| **`next_event`** | Low-level drive until one ordinary application event or a deadline; use for custom or multi-endpoint loops. |
| **`next_wake`** | Interruptible drive until an application event, capability progress, interruption, or deadline. |
| **Focused wait** | Recommended default for setup: drive until a specific result (`wait_peer_ready`, `wait_path`, `next_pubsub_event`, …) while buffering unrelated events. |
