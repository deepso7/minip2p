...
| Term | Meaning |
| --- | --- |
| **Endpoint** | App-facing entrypoint that owns identity, transports, and the swarm driver. Std form uses sockets; portable form is caller-driven. See [Concepts](/rust/guides/concepts#endpoint). |
...
| **Path** | NAT result: `DirectDialed`, `DirectPunched`, or `Relayed`. See [Traverse NAT](/rust/guides/traverse-nat). |
...
| **Compiled vs enabled** | A Cargo feature unlocks APIs; a builder method activates the driver. See [Install](/rust/quickstart/install#compiled-is-not-enabled). |
