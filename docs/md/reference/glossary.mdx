...
| **`next_event`** | Synchronous drive until one Endpoint event (capability variants included) or a deadline. Swallows interruption. Remains until #181. |
| **`next_wake`** | Migration wrapper over `wait` until #181. One Endpoint event (capability variants included), a deadline, or an interruption. Does not return driver-progress. |
