---
title: Node API
description: Concise reference for the high-level Node.js configuration, endpoint methods, streams, events, and public errors.
---
...
`@minip2p/core`. It is ESM-only and requires Node.js 24 or newer.
...
| `endpoint[Symbol.dispose]()` | Equivalent to `close()`; supports `using`. |
...
| `endpoint.setActive(active)` | Select active or idle driver polling. |

A started endpoint keeps the Node.js process alive until `close()`. See
[Endpoint lifecycle](/node/lifecycle) for signal handling and shutdown order.
...
Application code should stay on `@minip2p/node`. The adapter contract at
`@minip2p/core/backend` exists for platform adapter authors implementing
`Minip2pBackend`; it uses raw `{ tag, inner }` unions and native ownership
rules and is not an alternative application API.
