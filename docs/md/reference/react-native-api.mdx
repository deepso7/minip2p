...
| `listenAddr` | `string` | Local QUIC listen multiaddress; native defaults apply when omitted. |
| `protocols` | `readonly string[]` | Custom protocols accepted on inbound streams. |
| `relays` | `readonly string[]` | Direct QUIC relay peer addresses. |
...
| `dial` / `dialIp4` / `dialIp6` | Start direct-only QUIC dialing and return connection IDs. |
