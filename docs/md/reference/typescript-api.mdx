...
The TypeScript SDK is one API on two runtimes. Install `@minip2p/node` or
`@minip2p/react-native`; both re-export the platform-neutral public types and
errors from `@minip2p/core`.

Import application APIs from the runtime package:
...
Application code should stay on `@minip2p/node` or `@minip2p/react-native`. The
following exports exist for adapter and host integration work:

| Import | Audience |
| --- | --- |
| `@minip2p/core/backend` | Platform adapter authors implementing `Minip2pBackend`. |
| `@minip2p/react-native/native` | <Badge>React Native</Badge> Generated UniFFI/TurboModule integration and native stopped barriers. |
