...
| `listen` | `readonly [string, ...string[]]` | Listen multiaddresses; each address's shape selects QUIC or TCP. Omitted means dual-stack QUIC on ephemeral ports. |
...
`listen` is the only listen setting. A `/udp/<port>/quic-v1` address binds QUIC and a `/tcp/<port>` address binds TCP; a transport with no listen address is not bound. An explicit array replaces the defaults exactly, and QUIC accepts at most one listener per IP family.

```ts
// Omitted: QUIC on /ip4/0.0.0.0/udp/0/quic-v1 and /ip6/::/udp/0/quic-v1

// QUIC + TCP on fixed IPv4 ports
...
| `connectionInfo(peerId)` | Selected `{ connId, remoteAddr? }`, if connected. |
...
Promise operations use `{ timeoutMs?, signal? }`. Their default timeout is 65 seconds; `timeoutMs: 0` disables it. For Connection attempts a timeout ends only the wait (set `cancelOnTimeout: true` to also cancel), while an aborted `signal` cancels the attempt and the wait settles from its terminal.

| API | Result |
| --- | --- |
| `connect(target, options?)` | First usable path to a `ConnectTarget`: a peer ID, one complete peer address, or a list of complete addresses naming one peer. |
| `startConnect(target)` / `waitConnectResult(connectId, options?)` | Split-phase connection flow using a `connectId`. |
| `cancelConnect(connectId)` | Cancel an attempt; its terminal event settles any pending wait. |
...
| Relay and NAT | `reachabilityChanged`, `publicAddressesChanged`, `relayReserved`, `relayReservationLost`, `pathEstablished`, `pathUpgraded`, `holePunchFailed`, `connectFailed`, `connectCancelled`, `inboundDirectUpgrade` |
| Pubsub | `message`, `peerSubscribed`, `peerUnsubscribed`, `gossipsubOutboundFailure`, `gossipsubProtocolViolation` |
...
| `ConnectCancelledError` | An `AbortError`: the attempt ended with a `connectCancelled` terminal; includes `connectId` and `peerId`. |
| `ConnectResultLostError` | A dropped event carried the attempt's terminal; recover state with `connectedPeers()` / `path()`. |
