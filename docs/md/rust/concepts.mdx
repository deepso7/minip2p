...
| --- | --- | --- |
| Direct QUIC or TCP to a known address | `dial`, `dial_ip4`, or `dial_ip6` | connection, then `wait_peer_ready` if you need Identify |
...
Durable connection, peer, and listener-related state is available through State snapshot getters (`connected_peers`, `is_peer_ready`, `peer_info`, `connection_id`, `connection_remote_addr`, `listen_addresses`). `listen_addresses` reports transport-bound local addresses and can be non-empty before `listen` starts accepting. Those getters do not drive the endpoint, are not one cross-getter atomic snapshot, and may be ahead of the event stream.
