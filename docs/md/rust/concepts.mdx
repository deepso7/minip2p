...
An endpoint does not spawn a runtime. Use focused waits such as `wait_path` and `wait_peer_ready` when you need a particular milestone. Use `next_event` to handle application events, or `next_wake` when your loop also handles capability queues and interruptions. All these methods use transport readiness when supported. Each call drives only its own endpoint, so blocking on one endpoint can delay others sharing the same thread. `poll()` is the non-blocking step for an existing reactor.

```mermaid
flowchart TB
  A["Your caller-driven loop"] <-->|"wait_* / next_event / next_wake · events"| B["Endpoint"]
