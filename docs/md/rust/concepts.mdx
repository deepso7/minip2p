...
An endpoint does not spawn a runtime. Prefer `Endpoint::wait` for the ordered Endpoint event stream (event, deadline, or interrupted). If NAT, pubsub, discovery, or relay-server is enabled, keep using `next_wake` until capability events join the stream (#177) — `wait` does not wake on capability progress. Prefer `connect` plus `ConnectSettled` for a Connection attempt. Focused waits such as `nat_wait_path` and `wait_peer_ready`, plus `next_event` / `next_wake`, remain during migration. All these methods use transport readiness when supported. Each call drives only its own endpoint, so blocking on one endpoint can delay others sharing the same thread. `poll()` is the non-blocking step for an existing reactor.
...
| NAT-aware attempt with optional relay | `nat_connect`, `nat_connect_addr`, or `nat_connect_with_addrs` | `nat_wait_path` or `NatEvent` |
...
With the `nat` capability enabled, `nat_connect*` can race direct candidates against a relay leg. The first usable result is:
...
Ordinary connection, Identify, Ping, and application stream events leave through the Endpoint event stream: `Endpoint::wait`, `poll`, or the migration-era `next_event`. Prefer `wait` for new code — it returns an event, deadline, or interruption without swallowing interrupts, and does not use the old driver-progress wake. If NAT, pubsub, discovery, or relay-server is enabled, keep using `next_wake` until #177 — `wait` does not wake on capability progress.

Enabled capabilities still expose their own event queues until a later ticket folds them into the stream:

- `take_nat_events` / `next_nat_event`;
- `take_gossipsub_events` / `next_gossipsub_event`;
- `take_discovery_events` / `next_discovery_event`.

Durable connection, peer, and bound-address state is available through State snapshot getters (`connected_peers`, `is_peer_ready`, `peer_info`, `connection_id`, `connection_remote_addr`, `bound_addresses`). `bound_addresses` reports transport-bound local addresses and can be non-empty before `listen` starts accepting. Those getters do not drive the endpoint, are not one cross-getter atomic snapshot, and may be ahead of the event stream.
