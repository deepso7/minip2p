...
An endpoint does not spawn a runtime. Prefer readiness-driven waits for ordinary setup:

- `wait_path`, `wait_peer_ready`, or another focused wait for the milestone you need;
- `next_wake(deadline)` for one interruptible loop that reports capability queue progress;
- `next_event(deadline)` when you own a custom dispatcher or drive several endpoints on one thread;
- `poll()` for one non-blocking driving step inside an existing reactor.

```mermaid
flowchart TB
  A["Your caller-driven loop"] <-->|"wait_* / next_wake · events"| B["Endpoint"]
