...
Durable connection, peer, and listener state is available through State snapshot getters (`connected_peers`, `is_peer_ready`, `peer_info`, `connection_id`, `connection_remote_addr`, `listen_addresses`). Those getters do not drive the endpoint, are not one cross-getter atomic snapshot, and may be ahead of the event stream.
