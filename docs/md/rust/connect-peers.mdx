...
`wait_peer_ready` waits until Identify finishes so the dialer can print the peer and then measure Ping. That wait is a convenient milestone for this demo; `open_stream` does not require Identify for a protocol both peers already registered.
