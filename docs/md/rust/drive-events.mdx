...
Use `Endpoint::wait` for the ADR 0007 Endpoint wait outcomes (event, deadline, or interrupted). Use focused waits such as `wait_path` and `wait_peer_ready` when you need a particular milestone during migration. Use `next_event` for legacy application loops, or `next_wake` when your loop also handles capability queues and interruptions. All these methods use transport readiness when supported. Each call drives only its own endpoint, so blocking on one endpoint can delay others sharing the same thread.
...
| `wait_path` / `wait_peer_ready` / `wait_ping_rtt` | Drive until one milestone, buffering unrelated events. | Connection and setup milestones (migration) |
...
fn run(mut node: Endpoint, peer: minip2p::PeerId) -> Result<(), minip2p::Error> {
    ...
    loop {
        // `wait` may still return already-available events after the Instant
        // expires, so check the wall clock before each call.
