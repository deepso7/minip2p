...
An already-passed absolute deadline returns `Deadline` from `wait` / `next_wake` (or `None` from `next_event`) before delivering another queued event. The endpoint does not sleep or poll repeatedly past the deadline.
