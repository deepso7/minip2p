...
| --- | --- | --- |
| TCP | `cargo add minip2p-rs --features tcp` | Listen on a `/tcp` address with `.listen_on(...)`. |
| Relay paths and NAT traversal | `cargo add minip2p-rs --features nat` | Configure a relay, an AutoNAT server, or both. |
| Gossipsub | `cargo add minip2p-rs --features pubsub` | Call `.gossipsub()` or `.gossipsub_config(...)`. |
