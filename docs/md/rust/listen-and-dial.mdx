...
Prefer address-shaped listening: pass complete multiaddresses to `listen` (or `listen_default` for dual-stack QUIC) and call `bind`. The builder infers QUIC vs TCP from each address and groups compatible IPv4/IPv6 listeners onto one transport per shape.

| Method | Use it when |
| --- | --- |
| `.listen("/ip4/…/udp/…/quic-v1")?.bind()` | Configuration already uses complete multiaddresses. |
...
| `.listen(quic)?.listen(tcp)?.bind()` | One endpoint that speaks both. |
