...
`dial*` does not reserve on a relay, establish a circuit, or start DCUtR. When the application has enabled and configured NAT traversal, use `nat_connect*` plus `nat_wait_path` instead. See [Traverse NAT](/rust/traverse-nat).
