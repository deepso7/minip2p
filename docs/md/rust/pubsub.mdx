...
The `pubsub` feature exposes gossipsub. `EndpointBuilder::pubsub()` enables it with interoperable defaults and advertises `/meshsub/1.1.0` and `/meshsub/1.0.0`. You need a second peer on the same topic to observe delivery. See [Drive events](/rust/drive-events) for the caller-driven event model.
