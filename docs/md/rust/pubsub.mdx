...
loop {
    match node.wait(std::time::Duration::from_secs(5))? {
        minip2p::EndpointWaitOutcome::Event(minip2p::EndpointEvent::Gossipsub(event)) => {
            println!("{event:?}");
            break;
        }
        _ => {}
...
Gossipsub output arrives as `EndpointEvent::Gossipsub` on the Endpoint event stream. Discovery beacon-topic traffic never appears there. The migration APIs `take_gossipsub_events` / `next_gossipsub_event` remain until #181.
