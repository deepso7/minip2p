...
| --- | --- |
| Peer ID only; rely on configured relay | `nat_connect(&peer_id)` |
| One direct `PeerAddr`, optionally raced against relays | `nat_connect_addr(&peer_addr)` |
| Peer ID and several transport `Multiaddr` candidates | `nat_connect_with_addrs(peer_id, addrs)` |
...
let connect_id = node.nat_connect_addr(&target)?;
...
`nat_wait_path` returns `Ok(Some(path))` when it consumes the matching `NatEvent::PathEstablished`. It returns `Ok(None)` in two cases:
...
```mermaid
flowchart TB
  A["nat_connect*"] --> B["Direct candidates"]
