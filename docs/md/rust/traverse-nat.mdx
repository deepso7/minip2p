...
`wait_path` returns when traffic can flow. Keep calling readiness waits, `next_wake`, `next_event`, or `poll` afterward so DCUtR, reservation renewal, and connection events continue to progress. Prefer those readiness waits over alternating fixed short `next_event` polls on one thread.
