...
- the attempt failed; `NatEvent::ConnectFailed` stays queued for `take_nat_events` when the NAT leg failed (a Peer target with no route may settle only as `ConnectSettled`);
