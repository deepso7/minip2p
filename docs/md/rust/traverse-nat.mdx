...
use minip2p::{ConnectOutcome, EndpointEvent, EndpointWaitOutcome, NatEvent};
...
loop {
    match node.wait(deadline)? {
        ...
        EndpointWaitOutcome::Deadline => {
            eprintln!("no outcome before the deadline");
            break;
        }
    }
}
...
The migration helper `nat_wait_path(connect_id, deadline)` remains until #181. It returns `Ok(Some(path))` on the attempt's `PathEstablished`. If the attempt settles first, it returns the `path()` snapshot for a connected attempt or `Ok(None)` for a failed or cancelled one, leaving `ConnectSettled` queued for `wait`. It also returns `Ok(None)` when the deadline expires first.
