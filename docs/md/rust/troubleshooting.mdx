...
1. Confirm that both endpoints keep calling a focused wait (`nat_wait_path`, `wait_peer_ready`, …), `wait`, `next_event`, or `poll`. Each call drives only its own endpoint, so blocking on one endpoint can delay others sharing the same thread.
