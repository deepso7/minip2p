...
| A connection exists but an application protocol cannot open | Peer not connected, protocol unregistered, or remote rejects negotiation | Confirm `ConnectionEstablished`, matching protocol IDs, and keep both peers driving. `PeerReady` is optional; it only enables early `RemoteDoesNotSupport`. |
