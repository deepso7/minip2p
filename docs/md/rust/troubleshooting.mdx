...
1. Confirm that both endpoints keep calling a focused wait (`wait_path`, `wait_peer_ready`, …), `next_wake`, `next_event`, or `poll`. Each call drives only its own endpoint, so blocking on one endpoint can delay others sharing the same thread.
