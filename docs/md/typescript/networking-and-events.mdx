...
The TypeScript SDK starts its native driver inside `Minip2p.create`.
Configuration selects the network agents that driver coordinates; JavaScript
receives normalized, high-level events rather than raw native unions.
...
Android multicast
permissions](/typescript/setup-react-native#local-network-permissions).
