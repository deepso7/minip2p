...
`connectAddr` resolves when minip2p has a usable direct or relayed path. `waitPeerReady` waits for Identify and returns the peer's supported protocols. It is not required before `openStream` for a protocol you already know the remote speaks.
