...
const INTEROP_TIMEOUT: Duration = Duration::from_secs(30);
...
async fn main() -> Result<(), Box<dyn Error>> {
    tokio::time::timeout(INTEROP_TIMEOUT, run())
        .await
        .map_err(|_| Box::<dyn Error>::from("timed out waiting for relay interoperability"))?
}

async fn run() -> Result<(), Box<dyn Error>> {
    let relay_addr: Multiaddr = std::env::args()
        .nth(1)
        .ok_or("relay address argument missing")?
        .parse()?;
    ...
    macro_rules! verify_limit {
        ($limit:expr, $context:literal) => {{
            ...
            assert_eq!(
                limit.duration(),
                Some(Duration::from_secs(expected_duration))
            );
    ...
    while !(reserved && hop_advertised) {
        match destination.select_next_some().await {
            ...
            SwarmEvent::Behaviour(BehaviourEvent::Identify(identify::Event::Received {
                ...
                hop_advertised = info
                    .protocols
                    .iter()
                    .any(|protocol| protocol.as_ref() == HOP);
            }
            SwarmEvent::OutgoingConnectionError { error, .. } => {
                return Err(format!("reservation connection failed: {error}").into());
            }
            SwarmEvent::ListenerError { error, .. } => {
                return Err(format!("reservation listener failed: {error}").into());
            }
            SwarmEvent::ListenerClosed { reason, .. } => {
                return Err(format!("reservation listener closed: {reason:?}").into());
            }
            _ => {}
        }
    }
    ...
    while !(source_ping && destination_ping && outbound_limit && inbound_limit) {
        tokio::select! {
            event = source.select_next_some() => match event {
                ...
                SwarmEvent::Behaviour(BehaviourEvent::Relay(relay::client::Event::OutboundCircuitEstablished { relay_peer_id, limit })) if relay_peer_id == relay_peer => {
            ...
            event = destination.select_next_some() => match event {
                SwarmEvent::Behaviour(BehaviourEvent::Ping(ping::Event { peer, result: Ok(_), .. })) if peer == *source.local_peer_id() => destination_ping = true,
                SwarmEvent::Behaviour(BehaviourEvent::Relay(relay::client::Event::InboundCircuitEstablished { src_peer_id, limit })) if src_peer_id == *source.local_peer_id() => {
                    verify_limit!(limit, "inbound circuit");
                    inbound_limit = true;
                }
                SwarmEvent::IncomingConnectionError { error, .. } => return Err(format!("relayed STOP failed: {error}").into()),
                SwarmEvent::ListenerError { error, .. } => return Err(format!("relay listener failed: {error}").into()),
                SwarmEvent::ListenerClosed { reason, .. } => return Err(format!("relay listener closed: {reason:?}").into()),
                _ => {}
            }
        }
    }
    ...
    println!(
        "interop ok rust-libp2p={RUST_LIBP2P_REV} circuit-v2={CIRCUIT_V2_REV} reservation={reserved} identify_hop={hop_advertised} source_ping={source_ping} destination_ping={destination_ping} outbound_limit={outbound_limit} inbound_limit={inbound_limit} duration_limit={expected_duration} byte_limit={expected_bytes}"
    );
    Ok(())
}
